...
export class CWXHJSSdkConf {
    ...
    constructor (config) {
        ...
        // console.log('$$$$$$$', config.is_debug ,config.debug , '$$$$$$$')

        this.config.debug = undefined != config.is_debug ? config.is_debug :
                            undefined != config.debug ? config.debug : false //: true, // 开启调试模式,调用的所有api的返回值会在客户端alert出来，若要查看传入的参数，可以在pc端打开，参数信息会通过log打出，仅在pc端时才会打印。
