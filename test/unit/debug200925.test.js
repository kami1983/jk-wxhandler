...
describe('测试wxhandler 对象', () => {
     ...
     * */
    it('测试当没有设置opendebug时，promise传入的debug参数生效', async () => {
        ...
        expect(options_config_true.sdkconfig.opendebug).toBe(undefined)

        let oper = new CWXHandler(options_config_true)

        // 捕获wxsdk 对象
        await oper.catch_wxsdk().then((wxsdk) => {
            expect(wxsdk).toBeInstanceOf(CWXHJSSdk)
            expect(wxsdk.wxconfig).toBeInstanceOf(CWXHJSSdkConf)
            // 因为promise的回调中设置的是true
            expect(wxsdk.wxconfig.getDebugStatus()).toBe(true)
        }).catch((err)=>{
            throw err
        })

        // 如果添加opendebug 属性那么还是这个属性优先生效。
        options_config_true.sdkconfig.opendebug = false
        expect(options_config_true.sdkconfig.opendebug).toBe(false)
        let oper2 = new CWXHandler(options_config_true)
